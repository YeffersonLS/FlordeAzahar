    ...
    <div class="row">
        <div class="col-12">
            <div class="card border-primary">
                ...
                <div class="card-body">
                    <form action="{{ isset($editMode) ? route('admin.products.imagesPost', ['product' => $registro->t04id]) : route('admin.products.imagesPost') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @if (isset($editMode))
                            @method('PUT')
                        @endif
                                                ...
                                                @foreach ($imagenes_relacionadas as $imagen)
                                                    <tr id="imagen-{{ $imagen->id }}">
                                                        ...
                                                        <td>
                                                            ...
                                                            <button type="button" class="btn btn-danger btn-sm eliminar-imagen" data-productoid="{{ $registro->t04id }}" data-imagenid="{{ $imagen->r03id }}" data-toggle="modal" data-target="#modalConfirmarEliminarImagen"><i class="fa fa-trash"></i></button>
                                                        </td>
                                                    </tr>
                                                @endforeach
                                            </tbody>
...
<script>
    $(document).ready(function() {
        ...
        function eliminarImagen(productoId, imagenId) {
            $.ajax({
                url: '/eliminar-imagen/' + productoId + '/' + imagenId,
                type: 'GET',
